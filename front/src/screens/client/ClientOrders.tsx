...
type OrderItem = { id: number; product: Product; quantity: number; price: string | number; };
type Store = { id: number; final_name: string; };
type Order = { id: number; code: string; created_at: string; status: string; total: number; store: Store; items: OrderItem[]; };
  ...
  const renderOrder = ({ item }: { item: Order }) => (
    <Box borderWidth={1} borderColor="blue.300" borderRadius="lg" p={3} mb={4}>
      <Text bold fontSize="md" mb={2}>
        Pedido: {item.code} | Status: {getStatusLabel(item.status)} | Loja: {item.store.final_name} | Data do pedido: {new Date(item.created_at).toLocaleString()}
      </Text>
      <FlatList
        data={item.items}
